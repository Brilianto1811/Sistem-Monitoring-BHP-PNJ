...
@section('title', 'Halaman Pindah Bahan')
@section('content')
<div class="row">
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('transaksi.moving.create') }}" class="btn btn-primary m-1 radius-30 px-5"><i class="bx bx-plus me-1"></i>Form Pindah Bahan</a>
    </div>
</div>
<div class="row mt-3">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="card-title">
                    <h4 class="mb-0">Data Pindah Bahan</h4>
                </div>
                <hr/>
                        ...
                        <tbody>
                            @foreach($dataTransaksi as $nomor => $value)
                                <tr>
                                    ...
                                    <td>{{ $value->kode_pindah }}</td>
                                    <td>{{ $value->created_at }}</td>
                                    <td>{{ $value->user_create }}</td>
                                    <td>{!! $value->ket !!}</td>
                                    <td>
                                        <a href="{{ route('transaksi.moving.detail', ['id_pindah' => $value->id_pindah]) }}" class="btn btn-info btn-sm m-1"><i class="bx bx-detail"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
